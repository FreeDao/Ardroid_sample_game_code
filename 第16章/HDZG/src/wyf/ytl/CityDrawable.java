	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if(event.getAction() == MotionEvent.ACTION_DOWN){
			if(status == 0){//如果是询问状态时
				if(x>(DIALOG_BTN_START_X+ConstantUtil.LOX/ConstantUtil.RADIO)*ConstantUtil.RADIO 
						&& x<(DIALOG_BTN_START_X+DIALOG_BTN_WIDTH+ConstantUtil.LOX/ConstantUtil.RADIO)*ConstantUtil.RADIO
						&& y>(DIALOG_BTN_START_Y +ConstantUtil.LOY/ConstantUtil.RADIO)*ConstantUtil.RADIO
						&& y<(DIALOG_BTN_START_Y+DIALOG_BTN_HEIGHT+ConstantUtil.LOY/ConstantUtil.RADIO)*ConstantUtil.RADIO){//点下的是左键
				...
				else if(x>(DIALOG_BTN_START_X+DIALOG_BTN_SPAN+ConstantUtil.LOX/ConstantUtil.RADIO)*ConstantUtil.RADIO
						&& x<(DIALOG_BTN_START_X+DIALOG_BTN_SPAN+DIALOG_BTN_WIDTH+ConstantUtil.LOX/ConstantUtil.RADIO)*ConstantUtil.RADIO
						&& y>(DIALOG_BTN_START_Y+ConstantUtil.LOY/ConstantUtil.RADIO)*ConstantUtil.RADIO
						&& y<(DIALOG_BTN_START_Y+DIALOG_BTN_HEIGHT+ConstantUtil.LOY/ConstantUtil.RADIO)*ConstantUtil.RADIO){//点下的是攻城键
				...
				if(x>(DIALOG_BTN_START_X+ConstantUtil.LOX/ConstantUtil.RADIO)*ConstantUtil.RADIO 
					&& x<(DIALOG_BTN_START_X+DIALOG_BTN_WIDTH+ConstantUtil.LOX/ConstantUtil.RADIO)*ConstantUtil.RADIO
					&& y>(DIALOG_BTN_START_Y+ConstantUtil.LOY/ConstantUtil.RADIO)*ConstantUtil.RADIO
					&& y<(DIALOG_BTN_START_Y+DIALOG_BTN_HEIGHT+ConstantUtil.LOY/ConstantUtil.RADIO)*ConstantUtil.RADIO){//点下的是确定键
					recoverGame();//恢复游戏状态
				}
			}		
