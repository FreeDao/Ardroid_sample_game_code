...
public class ProcessView extends SurfaceView implements SurfaceHolder.Callback {
	...
	int process = 0;//0到processCount表示进度
	int startX = 90;
	int startY = 150;
	...
	public void onDraw(Canvas canvas){//自己写的绘制方法
		...
		//画的内容是z轴的，后画的会覆盖前面画的
		canvas.drawBitmap(processBackground, 0, 0, paint);//绘制背景图片
		canvas.drawBitmap(processMan, 10, 130, paint);//绘制人的图片
		canvas.drawBitmap(processBitmap, startX, startY, paint);
		canvas.drawRect(startX+process*(processBitmap.getWidth()/processCount), startY, startX+processBitmap.getWidth(), startY+processBitmap.getHeight(), paint);
		if(k == 0){
	...
	class TutorialThread extends Thread{//刷帧线程
		...
		public void run() {//重写的run方法
            ...
            while (this.flag) {//循环
                ...
                if(processView.process >= processCount){//当加载完成时
