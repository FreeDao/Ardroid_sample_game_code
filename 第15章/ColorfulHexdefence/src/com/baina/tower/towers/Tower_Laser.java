...
import com.baina.tower.bullet.Bullet_Laser;
...
public class Tower_Laser implements Tower,Externalizable {

	boolean flag = true;
	public int currentPrice = Constants.TOWER2CURRENTPRICE[0];
	float x;
	float y;//塔的中心点坐标
	float shotR=Constants.TOWER2_R[0];
	...
	public Tower_Laser(){};
	public Tower_Laser(float x, float y, Bitmap bitmap, GameSurfaceView mv) {
		super();
		this.x = x;
		this.y = y;
		this.all = bitmap;
	...
	public void fire(Monster a) {
		...
		if(limitCount%Constants.TOWER2_SHOOTTIME==0){
			MainGameActivity.sound.playMusic(Constants.jiguang, 0)  ;
			bulletList.clear();
			bulletList.add(new Bullet_Laser(x,y,a,master_list,currentState));
	...
	public void upDateSelf() {
		...
		case 1:
			//判断升级所需的钱数是否够用
			if(mv.getScore>=Constants.UPDATETOWER2[1])//此处的数字要写在常量列里面
			{
				//休息几秒钟有一个升级的过程,换图
				DrawShanXing();
				...
				mv.getScore -= Constants.UPDATETOWER2[1];
				
				//将射击的范围变大，伤害变大
				currentPrice = Constants.TOWER2CURRENTPRICE[currentState-1];
				shotR = Constants.TOWER2_R[currentState-1];
		...
		case 2:
			//判断升级所需的钱数是否够用
			if(mv.getScore>=Constants.UPDATETOWER2[2])//此处的数字要写在常量列里面
			{
				//休息几秒钟有一个升级的过程,换图
				DrawShanXing();
				MainGameActivity.sound.playMusic(Constants.towerupdate, 0)  ;
				...
				currentState = currentState + 1;
				mv.getScore -= Constants.UPDATETOWER2[2];
				//将射击的范围变大，伤害变大
				currentPrice = Constants.TOWER2CURRENTPRICE[currentState-1];
				shotR = Constants.TOWER2_R[currentState-1];
				//原先的类里面fire方法要重新改写一下，根据当前的状态子弹的数量增加
		...
		case 3:
			//判断升级所需的钱数是否够用
			if(mv.getScore>=Constants.UPDATETOWER2[3])//此处的数字要写在常量列里面
			{
				//休息几秒钟有一个升级的过程,换图
				DrawShanXing();
				MainGameActivity.sound.playMusic(Constants.towerupdate, 0)  ;
				...
				currentState = currentState + 1;
				mv.getScore -= Constants.UPDATETOWER2[3];
				//将射击的范围变大，伤害变大
				currentPrice = Constants.TOWER2CURRENTPRICE[currentState-1];
				shotR = Constants.TOWER2_R[currentState-1];
				//原先的类里面fire方法要重新改写一下，根据当前的状态子弹的数量增加
	...
	public void sell() 
	{
		mv.getScore += Constants.TOWER2CURRENTPRICE[currentState-1];
	}
