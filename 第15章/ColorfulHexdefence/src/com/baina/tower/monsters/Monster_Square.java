...
public class Monster_Square implements Monster,Externalizable{
	...
	static int UNIT_SPEED = Constants.MASTER1_UNIT_SPEED;//这个是地图两个点之间的分隔
	int BScount = 0;
	float pianyiX;
	...
	float totalBlood = Constants.MASTERNUM1BLOOD;//总血量
	float currentBlood = Constants.MASTERNUM1BLOOD;//当前怪的血量
	...
	public Monster_Square(){}//实现了Externalizable接口，必须有午餐构造器
	public Monster_Square(GameSurfaceView mv,Bitmap creep,float totalBlood)
	{
...
//					currentPointY-baoZhaShuZu[anmiIndex].getHeight()/2, paint);
	...
	public void run(){
			...
			}else
			{
		    	...
		    	{//动画播放完
		    		setLive(false);
		    		mv.getScore += Constants.KILLMASTER1GETSCORE;
	...
	public boolean decreaseBlood(float damage)
	{
		if(!anmiStart)
		{
			...
			{
				//血量没有时，播放动画
				Utils util = new Utils(1,currentPointX, currentPointY, 20,Color.RED,mv,4);
				mv.utils.add(util);
				util.start();
				MainGameActivity.sound.playMusic(Constants.sf_creep_die_0, 0)  ;
				anmiStart = true;
			}
		}
			return true;
