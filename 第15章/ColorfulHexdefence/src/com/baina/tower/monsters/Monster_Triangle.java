...
public class Monster_Triangle implements Monster,Externalizable{

	
	transient static int UNIT_SPEED = Constants.MASTER1_UNIT_SPEED;//这个是地图两个点之间的分隔
	boolean flag_controlDirection;
	float totalBlood = Constants.MASTERNUM2BLOOD;//总血量
	float currentBlood = Constants.MASTERNUM2BLOOD;//当前怪的血量
	boolean inCapability;
	
	public Monster_Triangle(){}//实现了Externalizable接口，必须有午餐构造器
	public Monster_Triangle(GameSurfaceView mv,Bitmap creep,float totalBlood)
	{
		this.mv = mv;
	...
	public void draw(Canvas canvas) {
		...
		if(!anmiStart)
		{
			...
			m1.setTranslate(currentPointX-creep.getWidth()/2,currentPointY-creep.getHeight()/2);
			
			m1.preRotate((float)Math.toDegrees(yAngle), creep.getWidth()/2, creep.getHeight()/2);
	        canvas.drawBitmap(creep, m1,null);
...
//					currentPointY-baoZhaShuZu[anmiIndex].getHeight()/2, paint);
	...
	public void run(){	
			...
			if(BScount>=UNIT_SPEED){
				...
				else
				{
					...
					if(bs>0){
						...
						float xspan=p2[0]-p1[0];
						float yspan=p2[1]-p1[1];
		...
		else
		{
	    	...
	    	{//动画播放完
	    		setLive(false);
	    		mv.getScore += Constants.KILLMASTER2GETSCORE;
	    		mv.totalScore+= Constants.KILLMASTER2GETSCORE;
	    	}
		}
	...
	public boolean decreaseBlood(float damage)
	{
		if(!anmiStart)
		{
			...
			{
				//血量没有时，播放动画
				Utils util = new Utils(1,currentPointX, currentPointY, 20,Color.YELLOW,mv,4);
				mv.utils.add(util);
				util.start();
				MainGameActivity.sound.playMusic(Constants.sf_creep_die_1, 0)  ;
				anmiStart = true;
			}
		}
			return true;
